<?php
	...
	add_action('factory_forms_000_plugin_created', 'factory_forms_000_plugin_created');
	function factory_forms_000_plugin_created($plugin)
	{
		$plugin->forms = new FactoryForms000_Manager($plugin);
	}

	class FactoryForms000_Manager {
	...
	class FactoryForms000_Form {
		...
		public function __construct($options = array(), $plugin = null)
		{
			...
			if( !FactoryForms000_Manager::$controlsRegistered ) {
				do_action('factory_forms_000_register_controls', $plugin);
				do_action('factory_forms_register_controls', $plugin);
				if( !empty($plugin) )
					do_action('factory_forms_register_controls_' . $plugin->pluginName, $plugin);
				FactoryForms000_Manager::$controlsRegistered = true;
			}
			...
			if( !FactoryForms000_Manager::$temper )
				FactoryForms000_Manager::$temper = $isFlat
					? 'flat'
					: 'volumetric';
		}
		 ...
		 *
		 * @since 1.0.0
		 * @param IFactoryForms000_ValueProvider $provider
		 * @return void
		 */
		...
		public function createControl($item)
		{
			...
			if( is_array($item) ) {
				...
				$controlData = FactoryForms000_Manager::$registeredControls[$item['type']];
		 ...
		 * @since 1.0.0
		 * @param type $item Item data.
		 * @return FactoryForms000_Holder A control holder object.
		 */
		public function createHolder($item)
		{
			...
			if( is_array($item) ) {
				...
				$holderData = FactoryForms000_Manager::$registeredHolders[$item['type']];
		...
		public function createCustomElement($item)
		{
			...
			if( is_array($item) ) {
				...
				$data = FactoryForms000_Manager::$registeredCustomElements[$item['type']];
		...
		public function html($options = array())
		{
			...
			if( !isset(FactoryForms000_Manager::$formLayouts[$this->formLayout]) ) {
				die(sprintf('[ERROR] The form layout %s was not found.', $this->formLayout));
			}

			// include a render code
			$layoutData = FactoryForms000_Manager::$formLayouts[$this->formLayout];
		...
		public static function connectAssetsForItem($item)
		{
			...
			if( self::isControl($type) ) {
				$haystack = FactoryForms000_Manager::$registerControls;
			} elseif( self::isControlHolder($type) )
				$haystack = FactoryForms000_Manager::$registeredHolders;
		...
		{
			return isset(FactoryForms000_Manager::$registeredControls[$item['type']]);
		}
		...
		{
			return isset(FactoryForms000_Manager::$registeredHolders[$item['type']]);
		}
		...
		{
			return isset(FactoryForms000_Manager::$registeredCustomElements[$item['type']]);
		}
	}