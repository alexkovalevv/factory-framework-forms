<?php
	 ...
	 * @since 3.1.0
	 */
	class FactoryForms000_PatternControl extends FactoryForms000_Control {
		...
		public function __construct($options, $form, $provider = null)
		{
			...
			$this->patterns = apply_filters('factory_forms_000_patterns', array());
			$this->patterns = apply_filters('factory_forms_000_patterns-' . $name, $this->patterns);
			$this->customPatterns = $this->getOption('patterns', array());

			$this->color = new FactoryForms000_ColorControl($options['color'], $form, $provider);
		}
