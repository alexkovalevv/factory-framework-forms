<?php
	...
	if( !class_exists('Wbcr_FactoryForms000_CheckboxControl') ) {

		class Wbcr_FactoryForms000_CheckboxControl extends Wbcr_FactoryForms000_Control {
			...
			protected function buttonsHtml()
			{
				...
				<div <?php $this->attrs() ?>>
					<button type="button" class="btn btn-default btn-small btn-sm factory-on <?php if( $value ) {
						echo 'active';
					} ?>"><?php _e('On', 'wbcr_factory_forms_000') ?></button>
					<button type="button" class="btn btn-default btn-small btn-sm factory-off <?php if( !$value ) {
						echo 'active';
					} ?>" data-value="0"><?php _e('Off', 'wbcr_factory_forms_000') ?></button>
